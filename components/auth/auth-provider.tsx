...
import { AuthContext, AuthError, AuthErrorCode, AuthState, AuthResponse } from '@/hooks/useAuth';
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const signIn = async (opts: { email: string; password: string; rememberMe?: boolean }): Promise<AuthResponse> => {
    try {
      ...
      // Attempt sign in with session persistence based on rememberMe
      const { data, error } = await supabase.auth.signInWithPassword({
        email: opts.email,
        password: opts.password,
        options: {
          persistSession: opts.rememberMe !== false // Default to true if not explicitly set to false
        }
      });
      if (error) {
